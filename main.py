...
def load_data(path, is_color=True):

    data = [cv2.imread(os.path.join(os.path, f), is_color) for f in os.listdir(path) if isfile(os.path.join(os.path, f))]
    data = np.array(data)
...
def sample_params(params):
    ...
    eps = K.random_normal(shape=(batch_size, latent_size), mean=0.0, stddev=1.0, seed=seed)
...
def build_vae(data, n_filters=[32, 64, 64], kernel_sizes=[8, 4, 3], strides=[4, 2, 1],
    ...
    z_mean = layers.Dense(latent_size, name='relu')(x)
    ...
    dec_output = layers.Conv2DTranspose(filters=3, kernel_size=1, strides=1, padding='same', activation='relu', name='decoder_output')(x)
    dec_output = layers.Cropping2D((3,0))(dec_output)
...
if __name__ == '__main__':
    ...
    vae, encoder, decoder, input, output, z_mean, z_logvar = build_vae(data)
    ...
    if args.weights:
        ...
        vae.fit(data,
                epochs=5,
                batch_size=32)
        vae.save_weights('vae_cnn_beamrider.h5')